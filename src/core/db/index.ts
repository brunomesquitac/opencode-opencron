...
const DEFAULT_DB_PATH = join(homedir(), '.local/share/opencode/opencron.db');
export const DB_FILE_PATH = process.env.OPENCRON_DB_PATH || DEFAULT_DB_PATH;
export { schema };
...
function initDb() {
    ...
    if (!_migrationRan) {
        ...
        try {
            ...
            console.error(`[opencron] migration failed: ${msg}`);
            throw new Error(`[opencron] DB migration failed: ${msg}`);
        }
    }
